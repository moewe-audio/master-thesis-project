...
static RmsTrackerController rmsCtrl(0.3, 90,0.01,0.0,0.0,1.0,SAMPLE_RATE);
...
void AudioCallback(AudioHandle::InputBuffer in, AudioHandle::OutputBuffer out, size_t size) {
    ...
    for (size_t i = 0; i < size; i++) {
        ...
        for (int m = 0; m < BANJO_FILTER.numModes; m++) {
            modalOut += banjoModes[m].process(softClipped);
        }
        modalOut -= softClipped;
        float gain = rmsCtrl.processSample(modalOut);
        float ampOut = gain * modalOut;
        out[0][i]    = piezoIn;
        out[1][i]    = ampOut;
        out[2][i]    = ampOut;
    }
