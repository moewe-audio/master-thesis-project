...
void AudioHandle::Impl::InternalCallback(int32_t* in, int32_t* out, size_t size)
{
    ...
    else if(audio_handle.callback_)
    {
        ...
        switch(bd)
        {
            // a marker for levin: one would think we would end up in this block, but you would be wrong. libDaisy only uses the main codec's bitdepth when used in conjuction
            case SaiHandle::Config::BitDepth::SAI_16BIT:
                for(size_t i = 0; i < size; i += 2)
                {
                ...
                break;
            case SaiHandle::Config::BitDepth::SAI_24BIT:
                // note for Levin: this is the block that will be used for ALL codecs
                for(size_t i = 0; i < size; i += 2)
                {
                    ...
                    if(chns > 2)
                    {
                        fin[2][i / 2] // This has been changed by Levin
                            = s162f(audio_handle.buff_rx_[1][offset + i])
                              * audio_handle.postgain_recip_;
                        fin[3][i / 2] // This as well
                            = s162f(audio_handle.buff_rx_[1][offset + i + 1])
                              * audio_handle.postgain_recip_;
                    }
                }
        ...
        switch(bd)
        {
            ...
            case SaiHandle::Config::BitDepth::SAI_24BIT:
                for(size_t i = 0; i < size; i += 2)
                {
                    ...
                    if(chns > 2)
                    {   // This is another case that has been changed by Levin
                        // The original code was using f2s24, which is not correct for the external 16 bit codec
                        audio_handle.buff_tx_[1][offset + i] = f2s16(
                            fout[2][i / 2] * audio_handle.output_adjust_);
